...
const Products = () => {
    ...
    useEffect(() => {
        fetch(`https://swap-it-server.vercel.app/category/${categoryName}`)
            .then(res => res.json())
            .then(data => {
                setProducts(data)
                if (data) {
                    fetch(`https://swap-it-server.vercel.app/veryfied/seller/${productEmail}`)
                        .then(res => res.json())
                        .then(data => {
                            console.log(data)
                            setVeryfied(data)
    ...
    const handleReport = product => {
        ...
        fetch(`https://swap-it-server.vercel.app/report-items`, {
            method: 'POST',
            headers: {
    ...
    return (
        <section className="dark:text-gray-100 min-h-screen">
