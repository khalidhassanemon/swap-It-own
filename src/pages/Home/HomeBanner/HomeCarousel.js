    ...
    return (
        <div id={`slide${id}`} className="carousel-item relative w-full">
            ...
            <div className="absolute flex justify-end transform -translate-y-1/2 left-5 right-5 bottom-0">
                <a href={`#slide${prev}`} className="rounded-full w-8 h-8 md:w-12 md:h-12 p-2 bg-gray-700 flex justify-center items-center text-white hover:bg-teal-700 mr-4">❮</a>
                <a href={`#slide${next}`} className="rounded-full w-8 h-8 md:w-12 md:h-12 p-2 bg-gray-700 flex justify-center items-center text-white hover:bg-teal-700">❯</a>
            </div>
            ...
            <div className="absolute flex justify-start transform -translate-y-1/3 left-5 mt-2 md:mt-0 md:left-10 top-3/4 md:top-2/4">
                <Link to='/' className="rounded cursor-pointer px-2 py-1 md:px-4 md:py-2  text-gray-100 font-semibold bg-teal-600 hover:bg-teal-700 ease-in duration-200 mr-5 mt-64">Discover More</Link>
            </div>
        </div >
    );
};
