...
const MyProducts = () => {
    ...
    const { data: myproducts = [], refetch, isLoading } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await fetch(`https://swap-it-server.vercel.app/products/${user?.email}`, {
                    headers: {
    ...
    const handleAdvertisement = product => {
        ...
        fetch(`https://swap-it-server.vercel.app/advertisements`, {
            method: 'POST',
            headers: {
    ...
    const handleDeleteProduct = product => {
        fetch(`https://swap-it-server.vercel.app/products/${product._id}`, {
            method: 'DELETE',
            headers: {
                'content-type': 'application/json',
                authorization: `bearer ${localStorage.getItem('accessToken')}`
