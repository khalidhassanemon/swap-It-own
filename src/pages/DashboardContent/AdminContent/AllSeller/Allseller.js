...
const Allseller = () => {
    ...
    const { data: allsellers = [], refetch, isLoading } = useQuery({
        queryKey: ['allseller'],
        queryFn: async () => {
            const res = await fetch('https://swap-it-server.vercel.app/users/allseller');
    ...
    const handleDeleteSeller = seller => {
        fetch(`https://swap-it-server.vercel.app/users/${seller._id}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleVerifySeller = seller => {
        fetch(`https://swap-it-server.vercel.app/users/seller/${seller._id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
